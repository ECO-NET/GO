	...
	public function import() {
		...
		if (isset($_POST['import_listings']) && $_POST['import_listings']) {
			...
			foreach ($posts AS $post) {
				...
				elseif ($level && !$level->eternal_active_period) {
					$expiration_date = w2dc_calcExpirationDate(current_time('timestamp'), $level);
		...
		if (isset($_POST['import_settings']) && $_POST['import_settings']) {
			...
			if (get_option('w2gm_fsubmit_addon')) {
				update_option('w2dc_fsubmit_moderation', get_option('w2gm_fsubmit_moderation'));
				update_option('w2dc_fsubmit_login_mode', get_option('w2gm_fsubmit_login_mode'));
				update_option('w2dc_fsubmit_edit_moderation', get_option('w2gm_fsubmit_edit_moderation'));
			...
			update_option('w2dc_google_api_key_server', get_option('w2dc_google_api_key_server'));
			...
			update_option('w2dc_search_bg_color', get_option('w2gm_search_bg_color'));
