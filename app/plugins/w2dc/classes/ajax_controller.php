		...
		switch ($post_args['controller']) {
			...
			case "listings_controller":
				...
				$out = array(
						...
						'map_markers' => ((!empty($post_args['with_map']) && $controller->google_map) ? $controller->google_map->locations_option_array : ''),
						'map_listings' => ((!empty($post_args['map_listings']) && $controller->google_map) ? $controller->google_map->buildListingsContent() : ''),
	...
	public function get_map_markers() {
		...
		if (isset($post_args['neLat']) && isset($post_args['neLng']) && isset($post_args['swLat']) && isset($post_args['swLng'])) {
			...
			if (!empty($post_args['map_listings'])) {
				$map_listings = $map_controller->google_map->buildListingsContent();
			}
		}
	...
	public function keywords_search() {
		...
		if ($validation->run()) {
			...
			while ($query->have_posts()) {
				...
				$listing_json['title'] = apply_filters('w2dc_listing_title_search_html', $title, $listing);
				$listing_json['name'] = $listing->title();
				$listing_json['url'] = get_the_permalink();
				$listing_json['icon'] = $listing->get_logo_url();
				$listing_json['sublabel'] = w2dc_crop_content(get_option('w2dc_excerpt_length'), get_option('w2dc_strip_excerpt'), false, false, '...');
