...
class w2dc_admin {
	...
	public function admin_enqueue_scripts_styles($hook) {
		...
		// include datepicker files in admin,
		// required for VC plugin, SiteOrigin plugin and widgets
		if (
			...
			wp_register_style('w2dc-jquery-ui-style', '//ajax.googleapis.com/ajax/libs/jqueryui/1.11.4/themes/smoothness/jquery-ui.css');
		...
		if (w2dc_isDirectoryPageInAdmin()) {
			...
			wp_register_style('w2dc_admin', W2DC_RESOURCES_URL . 'css/admin.css', array(), W2DC_VERSION);
			if (function_exists('is_rtl') && is_rtl()) {
				wp_register_style('w2dc_admin_rtl', W2DC_RESOURCES_URL . 'css/admin-rtl.css', array(), W2DC_VERSION);
			}

			if ($admin_custom = w2dc_isResource('css/admin-custom.css')) {
				wp_register_style('w2dc_admin-custom', $admin_custom, array(), W2DC_VERSION);
			}
...
';
	...
	public function render_palette_picker() {
		...
		if (!empty($w2dc_instance->frontend_controllers)) {
			if ((get_option('w2dc_compare_palettes') && current_user_can('manage_options')) || (defined('W2DC_DEMO') && W2DC_DEMO)) {
