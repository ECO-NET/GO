			...
			if (recalcPrice($listing->level->price) > 0) {
				...
				if ($invoice_id = call_user_func_array('w2dc_create_invoice', $invoice_args)) {
					if (is_user_logged_in()) {
						w2dc_addMessage(sprintf(__('New <a href="%s">invoice</a> was created successfully, listing become active after payment', 'W2DC'), w2dc_get_edit_invoice_link($invoice_id, 'url')));
					}
