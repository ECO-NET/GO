...
class w2dc_plugin {
	...
	public function init() {
		global $w2dc_instance, $w2dc_shortcodes, $w2dc_maps_styles, $wpdb;
		...
		$w2dc_maps_styles = apply_filters('w2dc_maps_styles', $w2dc_maps_styles);
	}
	...
	public function loadClasses() {
		...
		$this->updater = new w2dc_updater(plugin_basename(__FILE__));
	}
	...
	public function enqueue_scripts_styles($load_scripts_styles = false) {
		...
		if ((($this->frontend_controllers || $load_scripts_styles) && !$w2dc_enqueued) || get_option('w2dc_force_include_js_css')) {
			...
			wp_register_style('w2dc-jquery-ui-style', '//ajax.googleapis.com/ajax/libs/jqueryui/1.11.4/themes/' . $ui_theme . '/jquery-ui.css');
...
';
		echo 'var w2dc_map_markers_attrs = (function(map_id, markers_array, enable_radius_circle, enable_clusters, show_summary_button, show_readmore_button, draw_panel, map_style, enable_full_screen, enable_wheel_zoom, enable_dragging_touchscreens, center_map_onclick, map_attrs) {
		this.map_id = map_id;
...
';
		...
		echo 'var w2dc_google_maps_objects = ' . json_encode(
				array(
						...
						'map_style' => w2dc_getMapStyle(get_option('w2dc_map_style')),
