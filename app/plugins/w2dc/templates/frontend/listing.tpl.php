		...
		<div class="w2dc-listing-logo-wrap <?php if ($listing->logo_animation_effect): ?>w2dc-anim-style<?php else: ?>w2dc-no-anim-style<?php endif; ?>">
			...
			<figure class="w2dc-listing-logo <?php if ($listing->level->listings_own_page): ?>w2dc-listings-own-page<?php endif; ?>">
				...
				<div class="w2dc-featured-label"><?php _e("FEATURED", "W2DC"); ?></div>
				<?php endif; ?>
				
				<?php if (get_option('w2dc_listing_title_mode') == 'inside'): ?>
				<?php w2dc_renderTemplate('frontend/listing_header.tpl.php', array('listing' => $listing)); ?>
				<?php endif; ?>
				...
				<?php if ($listing->logo_image): ?>
					<?php $img_src = $listing->get_logo_url(array(800, 600)); ?>
				<?php else: ?>
		...
		<div class="w2dc-sticky-ribbon"><span><?php _e("Sticky", "W2DC"); ?></span></div>
		<?php endif; ?>

		<?php if ($w2dc_instance->getShortcodeProperty('webdirectory', 'is_favourites') && w2dc_checkQuickList($listing->post->ID)): ?>
